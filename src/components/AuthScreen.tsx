...
export default function AuthScreen({
  ...
  return (
    <div className="min-h-screen bg-slate-950 text-slate-100 flex items-center justify-center p-4 sm:p-6 md:p-8 font-sans transition-colors duration-300">
      <div className="relative w-full max-w-5xl bg-slate-900 border border-slate-800 rounded-3xl overflow-hidden shadow-2xl shadow-indigo-500/5 grid grid-cols-1 lg:grid-cols-12 min-h-[600px]">
        ...
        {/* Brand/Pitch Side Panel (4 cols) */}
        <div className="lg:col-span-5 bg-gradient-to-br from-slate-900 via-slate-950 to-indigo-950/40 p-8 sm:p-10 flex flex-col justify-between border-b lg:border-b-0 lg:border-r border-slate-800 relative overflow-hidden">
          ...
          <div className="space-y-6 z-10">
            ...
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-lg font-black font-sans tracking-tight text-white">
                  ECHELON
                </h1>
                <p className="text-[10px] text-emerald-400 font-mono tracking-wider uppercase font-semibold">Federation Level Portal</p>
              </div>
            </div>

            <div className="space-y-4 pt-4 sm:pt-6">
              <h2 className="text-2xl sm:text-3xl font-extrabold text-slate-100 font-sans tracking-tight leading-tight">
                Unlock Athletic Excellence.
              </h2>
              <p className="text-xs text-slate-400 leading-relaxed font-sans">
                A structured, high-performance coaching ecosystem connecting governing authorities, elite sporting clubs, competitive squads, coaches, and advancing athletes.
              </p>
            </div>
          </div>
          ...
          <div className="space-y-4 pt-8 sm:pt-0 z-10">
            <div className="text-[10px] font-mono text-indigo-400 uppercase tracking-widest font-bold">Structural Bounds</div>
            <div className="grid grid-cols-2 gap-2 text-left">
              <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800/85">
                <div className="text-xs font-bold text-slate-200">Multitenancy</div>
                <p className="text-[10px] text-slate-450 mt-1 leading-snug">Isolated association boundaries enforce data boundaries.</p>
              </div>
              <div className="bg-slate-900/50 p-3 rounded-xl border border-slate-800/85">
                <div className="text-xs font-bold text-slate-200">Rule Enforced</div>
                <p className="text-[10px] text-slate-450 mt-1 leading-snug">Secure cloud schemas validate athletic relationships.</p>
              </div>
            </div>

            <p className="text-[10px] font-mono text-slate-500 text-center lg:text-left mt-4 pt-4 border-t border-slate-800">
              Echelon Sports Portal — 2026 Sports Engineering Engine
            </p>
          </div>
        </div>
        ...
        <div className="lg:col-span-7 p-6 sm:p-10 flex flex-col justify-between space-y-8 z-10">
          ...
          <div className="flex justify-between items-center bg-slate-950 p-1.5 rounded-2xl border border-slate-850">
            <button
              onClick={() => { setIsSignUp(false); setErrorMsg(''); }}
              className={`flex-1 py-2.5 rounded-xl text-xs font-bold font-sans transition-all flex items-center justify-center gap-1.5 ${
                !isSignUp 
            ...
            {!isSignUp ? (
              ...
              <form onSubmit={handleSignInSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400 font-sans">Enter your registered credentials to enter the application portals</p>
                </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      ...
                      <input
                        ...
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 pl-10 pr-4 text-xs text-slate-100 placeholder:text-slate-650 outline-none focus:border-indigo-500 transition-colors font-sans"
                      />
                    </div>
                  </div>
                  ...
                  <div>
                    ...
                    <div className="relative">
                      <input
                        ...
                        placeholder="••••••••••••"
                        className="w-full bg-slate-950 border border-slate-800 rounded-xl py-3 px-4 text-xs text-slate-100 placeholder:text-slate-650 outline-none focus:border-indigo-500 transition-colors font-sans"
                      />
                      <button
                        type="button"
                ...
                <div className="flex items-center justify-between pt-2">
                  <label className="flex items-center gap-2 cursor-pointer select-none">
                    <input
                      ...
                      className="rounded border-slate-800 text-indigo-600 bg-slate-950 focus:ring-0 focus:ring-offset-0 h-4 w-4 cursor-pointer"
                    />
                    <span className="text-xs text-slate-400">Stay signed in on this device</span>
                  </label>
                  <a href="#forgot" className="text-xs text-indigo-400 hover:underline">Forgot?</a>
                </div>

                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-slate-100 hover:bg-slate-200 disabled:opacity-50 text-slate-950 font-bold text-xs py-3 rounded-xl transition duration-200 mt-4 flex items-center justify-center gap-1.5 shadow"
                >
                  {loading ? 'Authenticating Secures...' : 'Access Dashboard Portal'}
                  {!loading && <ArrowRight className="h-4 w-4 text-slate-950" />}
                </button>
              ...
              <form onSubmit={handleSignUpSubmit} className="space-y-4">
                <div className="space-y-1">
                  ...
                  <p className="text-xs text-slate-400">Join the Echelon sports hierarchy to participate in athletic operations</p>
                </div>
                ...
                <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-2">
                  <div>
                    ...
                    <input
                      ...
                      className="w-full bg-slate-950 border border-slate-800 rounded-xl p-3 text-xs text-slate-100 placeholder:text-slate-700 outline-none focus:border-indigo-500 transition-colors"
                    />
                  </div>
                  <div>
                    <label className="block text-[10px] font-mono text-slate-400 uppercase tracking-wider mb-1">EMAIL ADDRESS</label>
                ...
                <button
                  type="submit"
                  disabled={loading}
                  className="w-full bg-indigo-600 hover:bg-indigo-500 disabled:opacity-50 text-white font-bold text-xs py-3 rounded-xl transition duration-200 mt-4 flex items-center justify-center gap-1.5 shadow-lg shadow-indigo-600/15"
                >
